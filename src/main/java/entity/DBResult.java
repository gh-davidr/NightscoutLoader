<<<<<<< HEAD:src/main/java/entity/DBResult.java
package entity;
...
//			 "Event Time",
	...
	public DBResult(DBResult res, String device)
	{
		...
		try
		{
			...
			if (resType.equals("BG"))
			{
				...
				m_CP_Glucose = Double.valueOf(Double.parseDouble(res.getM_Result()));
			}
			else if (resType.equals("Carbs"))
			{
				...
				Double.valueOf(m_CP_Carbs = Double.parseDouble(res.getM_Result()));	
			}
			else if (resType.equals("Standard Bolus"))
			{
				...
				m_CP_Insulin = Double.valueOf(Double.parseDouble(res.getM_Result()));
			}
			...
			else if (resType.equals("MultiWave") || resType.equals("Extended Bolus Start"))
			{
				...
				if (res.getM_Duration() != null && !res.getM_Duration().equals(""))
				{
					...
					m_CP_Duration = Double.valueOf(Double.parseDouble(res.getM_Duration()));
				}
				...
				if (device.equals("Diasend")) {
					...
					try {
						...
						extendedinsulin = Double.valueOf(Double.parseDouble(res.getM_ExtendedAmount()));
					}
					catch (Exception e)
					{
					...
					if (res.getM_Duration() != null && !res.getM_Duration().equals(""))
					{
						if (extendedinsulin != 0.0 && m_CP_Duration > 0.0) {
							...
							m_CP_Relative = Double.valueOf(extendedinsulin / m_CP_Duration * 60);
					...
					if (res.getM_Enteredinsulin() != null && !res.getM_Enteredinsulin().equals(""))
					{
						...
						m_CP_Enteredinsulin = Double.valueOf(Double.parseDouble(res.getM_Enteredinsulin()));
						if (extendedinsulin != 0.0 && m_CP_Enteredinsulin > 0.0) {
							//							davidDiagnostics = "Setting m_CP_SplitExt - MultiWave";
							m_CP_SplitExt = Double.valueOf(extendedinsulin / m_CP_Enteredinsulin) * 100.0;
						...
						if (immediateinsulin != 0.0) {
							...
							m_CP_SplitNow = Double.valueOf(immediateinsulin / m_CP_Enteredinsulin) * 100.0;
			...
			else if (resType.equals("Tmp Basal Start"))
			{
				...
				m_CP_Percent  = Double.valueOf(Double.parseDouble(res.getM_Result()));     // Temp Basal
			}
			...
			{
				m_CP_EventType = "Temp Basal";
				m_CP_Duration = Double.valueOf(0);
				if (device.equals("Diasend")) {
					//					davidDiagnostics = "Setting m_CP_BasalValue - Basal";
					m_CP_BasalValue = Double.valueOf(Double.parseDouble(res.getM_Result()));// Diasend Temp Basal Absolute value used in CP
				}
				else {
					m_CP_Percent = Double.valueOf(0);
				}
				m_TmpBasal = true;
			}
	...
	public MergeResult merge(DBResult res, String device)
	{
		...
		long maxDiffBetweenSameCorrection = 1000 * 60 * 5;  // Allow up to 5 mins between BG & Ins
			...
			if ((diffMillies <= maxDiffBetweenSameCorrection) || (diffMillies <= maxDiffBetweenSameMealEvent && m_Carb))
			{
				...
				{
					m_BG = true;
					m_CP_Glucose = Double.valueOf(Double.parseDouble(res.m_Result));
				}
				// 28 Oct 2021 - Let's add the carb amounts together for the same time interval
				else if (resType.equals("Carbs"))
				{
					m_CP_Carbs = !m_Carb ? Double.valueOf(Double.parseDouble(res.m_Result)) : m_CP_Carbs + Double.valueOf(Double.parseDouble(res.m_Result));
				...
				// 28 Oct 2021 - Let's add the insulin amounts together for the same time interval
				else if (resType.equals("Standard Bolus"))
				{
					m_CP_Insulin = !m_Ins ? Double.parseDouble(res.m_Result) : m_CP_Insulin + Double.parseDouble(res.m_Result);
					m_Ins = true;
				}
				...
				else if (resType.equals("Pen Units"))
				{
					...
					m_CP_EventType = "Correction Bolus";
					m_CP_Insulin = !m_Ins ? Double.parseDouble(res.getM_Result()) : m_CP_Insulin + Double.parseDouble(res.getM_Result());
					m_CP_Notes += (m_CP_Notes.length() > 0 ? " : " : "") + "PenBolus";
					m_Ins = true;
				}

				// 28 Oct 2021 - Let's add the insulin amounts together for the same time interval
				else if ((resType.equals("MultiWave")) || (resType.equals("Combination")))
				{
					if (!m_Carb && m_BG)
					{
						m_Corr = true; // Allow a BG first then Ins
			...
			else if (m_TmpBasal == true && res.getM_ResultType().equals("Basal"))
			{
				...
				Double diffMilliesDbl = Double.valueOf(diffMillies);
	...
	public MergeResult merge_preOct2021(DBResult res, String device)
	{
		MergeResult result = MergeResult.Merged;  // Assume that the result cannot be merged
		...
		long maxDiffBetweenSameCorrection = 1000 * 60 * 5;  // Allow up to 5 mins between BG & Ins
			...
			if ((diffMillies <= maxDiffBetweenSameCorrection) || (diffMillies <= maxDiffBetweenSameMealEvent && m_Carb))
			{
				...
				{
					m_Carb = true;
					m_CP_Carbs = Double.valueOf(Double.parseDouble(res.m_Result));
					// However, if we see a Carb then it must be a Meal Bolus instead
					m_CP_EventType = "Meal Bolus";
...
>>>>>>> master:src/davidRichardson/DBResult.java
